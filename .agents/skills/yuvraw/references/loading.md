...
`FWicImageLoader` 在像素解码成功后枚举 frame 和 decoder 的 `IWICMetadataQueryReader`，
递归读取嵌套块；不依赖字段白名单，也不引入第三方库。相同路径和值只保留一份。
`FImageMetadata` 保存原查询路径、实际元数据块类型、已格式化值和不完整标记，经 `FImageData` 只读共享给 UI；
COM 对象在后台线程内释放，元数据随像素加载结果一起换代，`Clear()` 同时清除元数据。

IFD/EXIF/GPS 的 `VT_UI8/VT_I8` rational pair 按低 32 位分子、高 32 位分母解析，
有符号分子须独立转为 `int32_t`；向量逐项转换并跳过空字符串，避免手机 ASCII 的 NUL 填充变成逗号。
底层保留二进制原值，但浮层仅选取常见拍摄参数，不显示 MakerNote 或未知标签。
遍历深度、字段数、元素数和格式化文本均有独立上限；读取失败或超限标记不完整，不能导致已解码图片打开失败。
日志只记字段数、错误码和完成状态，不记录 GPS、序列号或元数据原文。

HEIC/HEIF 的元数据覆盖范围沿用系统 WIC codec；DNG/无头 RAW 加载器目前不提取这组元数据。
显示布局和交互见 [查看器与输入](viewer-and-input.md#exif-信息浮层)。
