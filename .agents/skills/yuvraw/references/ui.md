...
有独立重置图标按钮；六行统一以色块高度作为控件行高，标签、色块与方形按钮垂直对齐。
标题选中及悬停背景使用标题列的明确宽度，并禁用 Selectable 的半间距扩展；不绘制整行底色，
避免背景越过标题列侵入后方色块，各标题背景保持相同宽高。
`FUiIcons::ResetButton()` 复用逆时针回转箭头，跟随主题文字色及禁用透明度，悬停显示本地化的
“恢复默认值”。重置只恢复该基础色的默认值，派生状态会随之实时更新。

稀疏纹理阈值输入框关闭 `InputInt` 的加减步进按钮，右侧使用同一重置图标按钮，恢复默认 16384。
输入框与重置按钮一同跟随稀疏开关禁用；重置只修改草稿，应用后保存，取消则丢弃。
