...
请将 `0.0.24` 换成待发布版本。tag 推送后，无需本地编译或手动发布：GitHub 使用 Visual Studio 2026 仅构建 Release x64，执行现有回归测试及发行包校验，然后自动生成发布说明并发布正式 Release，附上前述三份文件。构建或校验失败时不会发布。实际 GPU/HDR 与无开发环境机器的验收仍按前文手动完成。
